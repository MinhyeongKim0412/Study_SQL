...
ALTER TABLE t_stock 
MODIFY COLUMN ITEMCODE 
varchar(30)

ALTER TABLE t_stock 
MODIFY COLUMN STOCKQTY 
varchar(10) COMMENT '재고수량' 
DEFAULT 0 

ALTER TABLE t_stock 
ADD CONSTRAINT t_stock_pk 
PRIMARY KEY (LOTNO,ITEMCODE)

ALTER TABLE t_stock 
ADD CONSTRAINT t_stock_t_itemmaster_fk 
FOREIGN KEY (ITEMCODE) 
REFERENCES t_itemmaster(ITEMCODE);
...
insert into t_customer (Cust_Id,Name,Phone) values  ('8','임재범',8888)
on DUPLICATE key 
update NAME = '이수',PHONE = 4444;
...
*** 트랜잭션 선언시  해당 의 테이블 을  해당 세션(접속 자) 이 점유 하게 된다. 
*/
